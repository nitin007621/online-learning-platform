...
function ChaptersTopicList({course}) {
  const courseLayout = course?.courseJson?.course;

  return (
    <div>
      <h2 className='font-bold text-4xl mt-10'>Chapters and Topics</h2>
      <div className="flex flex-col items-center justify-center mt-10">
      {courseLayout?.chapters.map((chapter, index) => (
      <div key={index} className="flex flex-col items-center">
      <div className="p-4 border shadow rounded-xl mt-5 bg-primary text-white">
          ...
          <div className="flex flex-col items-center" key={topicIndex}>
            ...
            {topicIndex === chapter.topics.length - 1 && (
              <>
                <div className="h-10 bg-gray-300 w-1"></div>
                <div className="flex items-center gap-5">
                  <div className="text-center rounded-full bg-gray-300 h-14 w-14 text-gray-500 p-4">
                    🎁 {/* Assuming this is an icon or a placeholder */}
                  </div>
                </div>
                <div className="h-10 bg-gray-300 w-1"></div>
              </>
...
export default ChaptersTopicList